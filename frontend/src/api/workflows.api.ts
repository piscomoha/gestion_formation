...
export interface FormateurStats {
  modules_affectes: number;
  groupes: number;
  stagiaires: number;
  moyenne_notes: number;
  total_notes: number;
  total_absences: number;
  presences: { present: number; absent: number; justifie: number; total: number };
  notes_par_module: Array<{ idModule: number; libelle: string; moyenne: number; total_notes: number }>;
}
...
export const workflowsApi = {
  ...
  async formateurStats(filters: WorkflowFilters) {
    const { data } = await apiClient.get<FormateurStats>("/formateur/stats", { params: filters });
    return unwrapApiData<FormateurStats>(data);
  },
  ...
  async markNotificationsRead() {
    const { data } = await apiClient.post("/notifications/read");
    return unwrapApiData(data);
  },
};

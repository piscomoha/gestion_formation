...
const navigation = [
  ...
  { label: "Mon bulletin", href: "/stagiaire/bulletin", icon: GraduationCap, roles: ["stagiaire"] },
];
...
export function Sidebar() {
  ...
  const items = navigation
    ...
    .map((item) => {
      if (item.label !== "Dashboard" || !user) return item;

      if (user.role === "formateur") {
        return { ...item, href: "/formateur/dashboard" };
      }

      if (user.role === "stagiaire") {
        return { ...item, href: "/stagiaire/dashboard" };
      }
      ...
      <nav className="space-y-1 p-4">
        {items.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground transition-all duration-200 ease-out hover:translate-x-1 hover:bg-muted hover:text-foreground",

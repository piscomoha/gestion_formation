...
export function StagiaireBulletinPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Mon bulletin" description="Consultez vos notes validées et votre historique de présence." />
      <Card>
      ...
      <div className="grid gap-4 xl:grid-cols-1">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Moyenne générale: {formatNumber(average, { maximumFractionDigits: 2 })}/20</CardDescription>
          </CardHeader>
          <CardContent className="overflow-x-auto p-0">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {bulletin.data?.notes.length === 0 && (
                  <tr>
                    <td colSpan={8} className="p-4 text-center text-muted-foreground">Aucune note disponible</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <CardContent className="overflow-x-auto p-0">
          <table className="w-full text-sm">
            ...
            <tbody>
              {presences.data?.presences.map((presence) => {
                ...
                return (
                  <tr key={presence.idPresence} className="border-b">
                    ...
                    <td className="p-3">{String(getValue(row, "affectation.module.libelle") ?? "-")}</td>
                    <td className="p-3">
                      <Badge variant={presence.statut === 'ABSENT' ? 'destructive' : presence.statut === 'PRESENT' ? 'default' : 'secondary'}>
                        {presence.statut}
                      </Badge>
                    </td>
              ...
              {presences.data?.presences.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-4 text-center text-muted-foreground">Aucun historique disponible</td>
                </tr>
              )}
            </tbody>
          </table>
